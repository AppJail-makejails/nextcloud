...
#  "$XYZ_DB_PASSWORD" from a file, especially for secrets)
file_env() {
    local var="$1"
...
# Write PHP session config for Redis to /usr/local/etc/php/redis-session.ini
...
if [ "$1" = "httpd-foreground" ]; then
    if [ -n "${APACHE_DISABLE_REWRITE_IP+x}" ]; then
        rm -f /usr/local/etc/apache24/Includes/remoteip.conf
    fi
fi

if [ "$1" = "httpd-foreground" ] || [ "$1" = "php-fpm" ] || [ "${NEXTCLOUD_UPDATE:-0}" -eq 1 ]; then
    uid="$(id -u)"
    gid="$(id -g)"
    if [ "$uid" = '0' ]; then
        case "$1" in
    ...
    (
        ...
        if [ -f /usr/local/www/html/version.php ]; then
            # shellcheck disable=SC2016
            installed_version="$(php -r 'require "/usr/local/www/html/version.php"; echo implode(".", $OC_Version);')"
        fi
        # shellcheck disable=SC2016
        image_version="$(php -r 'require "/nextcloud.src/version.php"; echo implode(".", $OC_Version);')"

        if version_greater "$installed_version" "$image_version"; then
            echo "Can't start Nextcloud because the version of the data ($installed_version) is higher than the OCI image version ($image_version) and downgrading is not supported. Are you sure you have pulled the newest image version?"
            exit 1
        fi
        ...
        if version_greater "$image_version" "$installed_version"; then
            ...
            rsync $rsync_options --delete --exclude-from=/upgrade.exclude /nextcloud.src/ /usr/local/www/html/
            for dir in config data custom_apps themes; do
                if [ ! -d "/usr/local/www/html/$dir" ] || directory_empty "/usr/local/www/html/$dir"; then
                    rsync $rsync_options --include "/$dir/" --exclude '/*' /nextcloud.src/ /usr/local/www/html/
                fi
            done
            rsync $rsync_options --include '/version.php' --exclude '/*' /nextcloud.src/ /usr/local/www/html/
            ...
            if [ "$installed_version" = "0.0.0.0" ]; then
                ...
                if [ -n "${NEXTCLOUD_ADMIN_USER+x}" ] && [ -n "${NEXTCLOUD_ADMIN_PASSWORD+x}" ]; then
                    ...
                    if [ "$install" = true ]; then
                        ...
                        max_retries=10
                        try=0
                        until  [ "$try" -gt "$max_retries" ] || run_as "php /usr/local/www/html/occ maintenance:install $install_options" 
                        do
			    ...
			    set -f # turn off glob
                            ...
                            for DOMAIN in ${NEXTCLOUD_TRUSTED_DOMAINS}; do
                                ...
                                run_as "php /usr/local/www/html/occ config:system:set trusted_domains $NC_TRUSTED_DOMAIN_IDX --value=\"${DOMAIN}\""
		...
		# not enough specified to do a fully automated installation 
            ...
            # Upgrade
                ...
                run_as 'php /usr/local/www/html/occ upgrade'
        ...
        if [ -n "${NEXTCLOUD_INIT_HTACCESS+x}" ] && [ "$installed_version" != "0.0.0.0" ]; then
            run_as 'php /usr/local/www/html/occ maintenance:update:htaccess'
        fi
    ) 9> /usr/local/www/html/nextcloud-init-sync.lock

    # warn if config files on persistent storage differ from the latest version of this image
    for cfgPath in /nextcloud.src/config/*.php; do
        cfgFile=$(basename "$cfgPath")

        if [ "$cfgFile" != "config.sample.php" ] && [ "$cfgFile" != "autoconfig.php" ]; then
            if ! cmp -s "/nextcloud.src/config/$cfgFile" "/usr/local/www/html/config/$cfgFile"; then
                echo "Warning: /usr/local/www/html/config/$cfgFile differs from the latest version of this image at /nextcloud.src/config/$cfgFile"
            fi
        fi
    done
